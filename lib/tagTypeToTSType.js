...
const tagTypeToTSType = (type, templates, renameMap, nonNullable = false) => {
  if (type.type === "NameExpression") {
    switch (type.name) {
      ...
      default: {
        const name = typeNameMap[type.name] || rename(renameMap, type.name);
  ...
  if (type.type === "UnionType") {
    ...
    const types = elements.reduce((types, t) => {
      let tsType = tagTypeToTSType(t, templates, renameMap, nonNullable);
  ...
  if (type.type === "TypeApplication") {
    ...
    const tsType = j.tsTypeReference(
      j.identifier(typeNameMap[type.expression.name] || type.expression.name),
      j.tsTypeParameterInstantiation(
        type.applications.map(t =>
          tagTypeToTSType(t, templates, renameMap, true)
        )
      )
    );
